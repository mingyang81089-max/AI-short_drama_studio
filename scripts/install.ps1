...
function Assert-EnvFile {
  ...
  $message = if (Test-Path -LiteralPath $EnvExamplePath) {
    "Missing .env. Please fill it in manually from .env.example, then rerun scripts/install.ps1."
  } else {
    "Missing .env. Create it manually, then rerun scripts/install.ps1."
  }
...
try {
  ...
  Invoke-Compose -Arguments @("run", "--rm", "web", "pnpm", "db:migrate")

  Write-Host "Running database seed..."
  Invoke-Compose -Arguments @("run", "--rm", "web", "pnpm", "db:seed")

  Write-Host "Starting web and worker..."
  Invoke-Compose -Arguments @("up", "-d", "web", "worker")
